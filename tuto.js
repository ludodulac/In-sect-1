...
const SYM = {
  queen:'🐝', spider:'🕷️', fly:'🪰', beetle:'🪲', lady:'🐞', ant:'🐜',
};
...
const SLIDES = [
  ...
  /* 1 — GUÊPE REINE */
  {
    type: 'piece', sym: '🐝', name: `La Guêpe Reine`,
  ...
  /* 2 — ARAIGNÉE */
  {
    type: 'piece', sym: '🕷️', name: `L'Araignée Assassine`,
  ...
  /* 3 — MOUCHE */
  {
    type: 'piece', sym: '🪰', name: `La Mouche Journaliste`,
  ...
  /* 4 — SCARABÉE */
  {
    type: 'piece', sym: '🪲', name: `Le Scarabée Déplaceur de Dépouilles`,
  ...
  /* 5 — COCCINELLE */
  {
    type: 'piece', sym: '🐞', name: `La Coccinelle Déplaceuse de Vivant`,
  ...
  /* 6 — FOURMI */
  {
    type: 'piece', sym: '🐜', name: `Les Fourmis Soldates`,
...
function buildQueenAnim() {
  return {
    piece:{r:2,c:2,color:'yellow',sym:'🐝'},
    enemy:{r:0,c:4,color:'red',sym:'🐜'},  // fourmi ennemie
    frames:[
      ...
      {delay:1500, label:{text:'Dépouille repositionnée sur n\'importe quelle case libre.', type:'corpse', dur:2000}, cells:[], piece:null, placeCorpse:{r:3,c:1,sym:'🐜'}},
      // Reset
...
function buildSpiderAnim() {
  return {
    piece:{r:2,c:1,color:'yellow',sym:'🕷️'},
    enemy:{r:2,c:4,color:'blue',sym:'🐞'},  // coccinelle ennemie
    frames:[
      // Montre portée dame
      {delay:500, label:{text:'Parcourt lignes et diagonales sans limite de portée.', type:'move'}, cells:[
      ...
      {delay:1800, label:{text:'Élimine la cible puis occupe immédiatement sa case.', type:'kill'}, cells:[], piece:{r:2,c:4}, killEnemy:true, corpseAt:{r:2,c:1,sym:'🐞'}},
...
function buildFlyAnim() {
  ...
  return {
    piece:{r:4,c:4,color:'yellow',sym:'🪰'},
    enemies:[
      {r:1,c:2,color:'red',   sym:'🐜'}, // ortho haut
      {r:2,c:1,color:'blue',  sym:'🕷️'}, // ortho gauche
      {r:2,c:3,color:'green', sym:'🐝'}, // ortho droite
      {r:3,c:2,color:'red',   sym:'🪲'}, // ortho bas
    ],
    enemiesDiag:[
      {r:1,c:1,color:'red',   sym:'🐜'},
      {r:1,c:3,color:'blue',  sym:'🕷️'},
      {r:3,c:1,color:'green', sym:'🐝'},
      {r:3,c:3,color:'red',   sym:'🪲'},
    ],
    flyPhase: true,
    frames:[
...
function buildBeetleAnim() {
  ...
  return {
    piece:{r:2,c:0,color:'yellow',sym:'🪲'},
    corpse:{r:0,c:2,sym:'🐜'}, // dépouille fourmi — accessible en diag ↗
    frames:[
...
function buildLadybugAnim() {
  return {
    piece:{r:2,c:1,color:'yellow',sym:'🐞'},
    enemy:{r:2,c:3,color:'red',sym:'🪰'}, // mouche ennemie
    frames:[
      // Montre portée dame
      {delay:500, label:{text:'Parcourt lignes et diagonales sans limite de portée.', type:'move'}, cells:[
        {r:0,c:1,cls:'t-move'},{r:1,c:1,cls:'t-move'},{r:3,c:1,cls:'t-move'},{r:4,c:1,cls:'t-move'},
...
function buildAntAnim() {
  ...
  return {
    piece:{r:3,c:3,color:'yellow',sym:'🐜'},
    enemy:{r:1,c:3,color:'blue',sym:'🪲'},
    frames:[
      ...
      {delay:1800, label:{text:'Dépouille repositionnée.', type:'corpse', dur:1800}, cells:[], piece:null, placeCorpse:{r:5,c:0,sym:'🪲'}},
      // Reset
...
function _buildContent(el, slide, idx) {
  ...
  el.innerHTML=`
    <div class="tuto-piece-header">
      <div class="tuto-piece-sym">${slide.sym}</div>
...
function _placePiece(board, state) {
  ...
  el.textContent=state.piece.sym||'🐝';
}
...
function _placeEnemy(board, enemy, eid, diagIdx) {
  ...
  const fs=parseFloat(size)*.55;
  el.style.left=left; el.style.top=top; el.style.width=size; el.style.height=size; el.style.fontSize=fs+'px';
  el.textContent=enemy.sym||'🐛';
  board.appendChild(el);
}
