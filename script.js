...
const STRIPE_URL = "https://buy.stripe.com/XXXXXXXX";
...
async function doShare() {
  const shareData = {
    title: 'IN-SECT — Guerre des Colonies',
...
function goPremium() {
  window.location.href = STRIPE_URL +
    (STRIPE_URL.includes('?') ? '&' : '?') +
    'success_url=' + encodeURIComponent(window.location.href + '?premium=true');
}

// ── Tentative de lancer une partie ──
// Appelé par le bouton JOUER et par le bouton REJOUER
function tryStartGame() {
  if (hasAccess()) {
    ...
    // Plus de parties — afficher la popup freemium
    showModal('freemium');
    updateShareDots();
  }
}
...
function toggleMute() {} // stub — suppression future si besoin
...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
(function(){
  const cv = document.getElementById('c-ambient'), ctx = cv.getContext('2d');
  let W, H, pts = [];
...
function updateModePreview(n) {
  ...
  if (n === 1) {
    el.innerHTML = `<svg class="preview-svg" viewBox="0 0 72 72">
      ...
      <text x="16" y="20" text-anchor="middle" font-size="7">🐝</text>
      <circle cx="56" cy="56" r="6" fill="#6A0010" stroke="rgba(255,80,100,.6)" stroke-width="1.2"/>
      <text x="56" y="60" text-anchor="middle" font-size="7">🐝</text>
    ...
    el.innerHTML = `<svg class="preview-svg" viewBox="0 0 72 72">
      ...
      <text x="60" y="16" text-anchor="middle" font-size="6">🐝</text>
      <circle cx="12" cy="60" r="6" fill="#001A60" stroke="rgba(64,144,255,.7)" stroke-width="1"/>
      <text x="12" y="64" text-anchor="middle" font-size="6">🐝</text>
      <circle cx="60" cy="60" r="6" fill="#6A0010" stroke="rgba(255,48,80,.7)" stroke-width="1"/>
      <text x="60" y="64" text-anchor="middle" font-size="6">🐝</text>
...
function drawPiece(cv, color, sym, dead, selected) {
  ...
  if (dead) {
    ...
    ctx.strokeStyle = 'rgba(255,255,255,0.8)'; ctx.lineWidth = w*.06; ctx.stroke();
    ctx.font = `${w*.55}px serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.globalAlpha = 0.3; ctx.fillText(sym, cx, cy+w*.02); ctx.globalAlpha = 1;
  } else {
    ...
    ctx.font = `${w*.60}px serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.shadowColor = 'rgba(0,0,0,.85)'; ctx.shadowBlur = w*.07;
    ctx.fillText(sym, cx, cy+w*.02); ctx.shadowBlur = 0;
  }
}
...
function renderPlayers() {
  ...
  for (const c of G.order) {
    ...
    card.innerHTML = `<div class="pch"><div class="pdot"></div><div class="pname">${c.toUpperCase()}</div><div class="ptag">${tag}</div></div><div class="ppieces">${myPieces.map(p=>`<span class="${p.dead?'ppid':'ppi'}" title="${PNAME[p.type]}">${SYM[p.type]}</span>`).join('')}</div>`;
    pp.appendChild(card);
  }
}
...
function updateTurnUI() {
  ...
  if (pm) { pm.textContent = msg; pm.classList.toggle('visible', !!msg); }
}

function updatePieceInfo(piece) {
  const pi = document.getElementById('pi');
  pi.innerHTML = `<div class="pii">${SYM[piece.type]}</div><div style="flex:1;min-width:0;"><div class="pin" style="color:${CCSS[piece.color]}">${PNAME[piece.type]}</div><div class="pid">${PDESC[piece.type]}</div></div>`;
...
function endGame(winner) {
  ...
  setTimeout(()=>{
    ...
    setText('ewinner', winner?`${SYM['chef']} ${CNAME[winner]} remporte la guerre.`:'');
    setText('end-turns',_gameTurns); setText('end-caps',_gameCaps);
    showScreen('end');
  },400);
}
...
function finishTurn(){
  ...
  if(twoPlayer){
    ...
    // Ordre exemple si Jaune sur le Nid : Jaune→Vert→Jaune→Rouge→Jaune→Bleu→Jaune→...
    if(G.labActive&&G.labActive!==justPlayed){
      G.labExtra=G.order.indexOf(G.labActive);
      if(G.labExtra>=0){
        G.idx=G.labExtra;
        renderBoard();renderPlayers();updateTurnUI();
    ...
    } else if(G.labActive&&G.labActive===justPlayed){
      let nextIdx=(G.idx+1)%G.order.length;
      if(G.order.length>1&&G.order[nextIdx]===G.labActive){nextIdx=(nextIdx+1)%G.order.length;}
      G.idx=nextIdx;
      renderBoard();renderPlayers();updateTurnUI();
...
setTimeout(() => {
  ...
  updateModePreview(1);
  updateStatusBar(); // affiche le compteur de parties
}, 2000);

// Déverrouillement du contexte audio au premier clic
